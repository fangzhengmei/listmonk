...
**统计**：35 个语言文件（含基准语言 en.json）

### 2.2 计数依据（可复核）

**统计口径说明**：

1. **目录范围**：仅统计 `i18n/` 目录下的 `.json` 文件
2. **包含基准**：包含 `en.json`（基准语言文件）
3. **排除范围**：不包含其他目录的 JSON 文件（如 `project.inlang.json`、`frontend/jsconfig.json` 等）

**LS 输出逐行计数证据**（来自 `LS i18n/`）：
...
**复核方式**：
- 方式 1：运行 `LS i18n/` 逐行计数
- 方式 2：运行 `Glob i18n/*.json` 获取文件列表后计数
...
| 校正项 | 原报告 | 校正后 | 证据位置 |
|--------|--------|--------|----------|
| 语言文件数量 | 40+ 种 → 37 个文件 | **35 个文件** | `LS i18n/` 逐行计数 |
...
| 复核方式 | 步骤 | 预期结果 |
|----------|------|----------|
| 方式 1：LS 目录 | 运行 `LS i18n/` 后逐行计数 | 35 行（文件） |
| 方式 2：Glob 模式 | 运行 `Glob i18n/*.json` | 35 个文件 |
| 方式 3：手动列表 | 对照本报告 2.2 节的序号表 | 1-35 共 35 项 |
...
*报告校正时间：2026-05-05*
*分析版本：基于当前代码库实际状态*
*校正内容：语言数量（35）、日语代码、键顺序行为、三阶段证据对照、计数依据*
